...
static int kd_tree_split_node(kd_node_t *node, int dimensions, int levels_remaining, int min_per_node) {
    ...
    if( node->left->num_items < node->num_items )
        kd_tree_split_node(node->left, dimensions, levels_remaining-1, min_per_node);
    if( node->right->num_items < node->num_items )
        kd_tree_split_node(node->right, dimensions, levels_remaining-1, min_per_node);

    return 0;
}

int kd_tree_build(kd_tree_t *tree, kd_item_t *items, int n, int dimensions) {
    ...
    return kd_tree_split_node(tree->root, dimensions, -1, -1);
}
